...
def main():
    res_list = get_full_data()
    if res_list == None:
        print('Add please new adresses!')
        return

    wb = openpyxl.load_workbook('output_data.xlsx')
    sheet = wb['data']
    for i in range(len(res_list)):
        sheet[f"C{res_list[i][-1]}"].value = res_list[i][1]
        sheet[f"D{res_list[i][-1]}"].value = res_list[i][2]
...
#['nuplantcare.com', 'https://nuplantcare.com/ads.txt', 'Ezoic', 3825]